...
via-drill-guide-generator#1 (pill/oval/rect drills with `hole_width`/`hole_height` are cut instead of dropped),
...
route-styler#1 (unrouted `pcb_trace` entries that omit `route` beautify and visualize instead of throwing),
remove-prefab-vias#1 (unrouted `pcb_trace` entries that omit `route` skip hole matching instead of throwing).
